// アートワーク抽出の検証スクリプト
//
// ビルド: rustc --edition 2021 verify-artwork.rs -o verify-artwork \
//          -L ../../target/release/deps \
//          --extern id3=../../target/release/deps/libid3-*.rlib \
//          --extern mp4ameta=../../target/release/deps/libmp4ameta-*.rlib \
//          --extern anyhow=../../target/release/deps/libanyhow-*.rlib
//
// または: cargo build --release && cargo run --bin verify-artwork -- <file_path>

use std::env;
use std::fs;
use std::path::Path;

fn extract_artwork(file_path: &str) -> Result<Option<Vec<u8>>, Box<dyn std::error::Error>> {
